...
{ id: 'uid', label: 'U Id', visible: true},
{ id: 'projectID', label: 'Project ID', visible: true},
{ id: 'projectName', label: 'Project Name', visible: true},
{ id: 'licenseAgreementType', label: 'License Agreement Type', visible: true},
{ id: 'complianceRequired', label: 'Compliance Required', visible: true},
{ id: 'owner', label: 'Owner', visible: true},
{ id: 'expiryDate', label: 'Expiry Date', visible: true},
{ id: 'documentPath', label: 'Document Path', visible: true},
{ id: 'documentURL', label: 'Document URL', visible: true},
...
{ id: 'updatedDate', label: 'Updated Date', visible: true},
    ...
    const handleSearch = (e?: React.FormEvent) => {
        ...
        const apiUrl = `${config.API_URL_APPLICATION1}/SCMaster/GetSC/${query}`;
        console.log(apiUrl)
        axios.get(apiUrl, { headers: { 'accept': '*/*' } })
            .then((response) => {
                console.log("search response ", response.data.scMasters);
                setMesses(response.data.scMasters)
    ...
    const fetchRoles = async () => {
        setLoading(true);
        try {
            const response = await axios.get(`${config.API_URL_APPLICATION1}/SCMaster/GetSC`, {
    ...
    useEffect(() => {
        ...
        // fetchData('SCMaster/GetSC', setMessList, 'scMasters');
    ...
    return (
        <>
            <div className="d-flex bg-white p-2 my-2 justify-content-between align-items-center">
                <span><i className="ri-file-list-line me-2 text-dark fs-16"></i><span className='fw-bold text-dark fs-15'>SC Master</span></span>
                                        ...
                                        messes.slice(0, 10).map((item, index) => (
                                            <tr key={item.id}>
                                                ...
                                                {(role === 'Admin' || role === 'DME') && (
                                                    ...
                                                    <td><Link to={`/pages/SCMasterAddEdit/${item.id}`}>
