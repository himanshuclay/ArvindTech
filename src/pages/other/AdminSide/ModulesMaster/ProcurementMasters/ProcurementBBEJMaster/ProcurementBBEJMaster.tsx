...
const ProcurementBBEJMaster = () => {
    ...
    const [columns, setColumns] = useState<Column[]>([
        ...
        { id: 'pier_Span_ChainageNo', label: 'pier_Span_ChainageNo', visible: true},
        { id: 'drawingBroadCategory', label: 'drawingBroadCategory', visible: true},
        { id: 'completionDate', label: 'completionDate', visible: true},
        { id: 'dtnNo', label: 'dtnNo', visible: true},
        { id: 'initiationType', label: 'initiationType', visible: true},
        { id: 'reconcileQtyPR', label: 'reconcileQtyPR', visible: true},
        { id: 'reconcileQtyPO', label: 'reconcileQtyPO', visible: true},
        { id: 'reconsileShortCloseQty', label: 'reconsileShortCloseQty', visible: true},
        { id: 'reconsileDespatchedQty', label: 'reconsileDespatchedQty', visible: true},
    ...
    const handleSearch = (e?: React.FormEvent) => {
        ...
        const apiUrl = `${config.API_URL_APPLICATION1}/ProcurementBBEJMaster/GetProcurementBBEJ/${query}`;
        console.log(apiUrl)
        axios.get(apiUrl, { headers: { 'accept': '*/*' } })
            .then((response) => {
                console.log("search response ", response.data);
                setMesses(response.data.procurementBBEJMasters)
    ...
    const fetchRoles = async () => {
        setLoading(true);
        try {
            const response = await axios.get(`${config.API_URL_APPLICATION1}/ProcurementBBEJMaster/GetProcurementBBEJ`, {
    ...
    useEffect(() => {
        ...
        // fetchData('ProcurementBBEJMaster/GetProcurementBBEJ', setMessList, 'procurementBBEJMasters');
    ...
    return (
        <>
            <div className="d-flex bg-white p-2 my-2 justify-content-between align-items-center">
                <span><i className="ri-file-list-line me-2 text-dark fs-16"></i><span className='fw-bold text-dark fs-15'>ProcurementBBEJ Master</span></span>
                <div className="d-flex justify-content-end  ">
                    ...
                    {(role === 'Admin' || role === 'DME') && (
                        ...
                        <Link to='/pages/ProcurementBBEJMasterAddEdit'>
                            <Button variant="primary" className="me-2">
                                Add ProcurementBBEJ Master
                            </Button>
                        </Link>)}
                                        ...
                                        messes.slice(0, 10).map((item, index) => (
                                            <tr key={item.id}>
                                                ...
                                                {(role === 'Admin' || role === 'DME') && (
                                                    ...
                                                    <td><Link to={`/pages/ProcurementBBEJMasterAddEdit/${item.id}`}>
...
export default ProcurementBBEJMaster;