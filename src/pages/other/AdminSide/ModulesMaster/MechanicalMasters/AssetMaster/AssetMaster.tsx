...
const AssetMaster = () => {
    ...
    const handleSearch = (e?: React.FormEvent) => {
        ...
        const apiUrl = `${config.API_URL_APPLICATION1}/AssetMaster/GetAsset/${query}`;
        console.log(apiUrl)
        axios.get(apiUrl, { headers: { 'accept': '*/*' } })
            .then((response) => {
                console.log("search response ", response.data.assetMasters);
                setMesses(response.data.assetMasters)
    ...
    const fetchRoles = async () => {
        setLoading(true);
        try {
            const response = await axios.get(`${config.API_URL_APPLICATION1}/AssetMaster/GetAsset`, {
    ...
    useEffect(() => {
        ...
        fetchData('AssetMaster/GetAsset', setMessList, 'assetMasters');
    ...
    return (
        <>
            <div className="d-flex bg-white p-2 my-2 justify-content-between align-items-center">
                <span><i className="ri-file-list-line me-2 text-dark fs-16"></i><span className='fw-bold text-dark fs-15'>Asset Master</span></span>
                <div className="d-flex justify-content-end  ">
                    ...
                    {(role === 'Admin' || role === 'DME') && (
                        ...
                        <Link to='/pages/AssetMasterAddEdit'>
                                        ...
                                        messes.slice(0, 10).map((item, index) => (
                                            <tr key={item.id}>
                                                ...
                                                {(role === 'Admin' || role === 'DME') && (
                                                    ...
                                                    <td><Link to={`/pages/AssetMasterAddEdit/${item.id}`}>
