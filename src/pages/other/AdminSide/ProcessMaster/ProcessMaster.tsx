...
const ModuleMaster = () => {
	...
	const handleSearch = (e?: React.FormEvent) => {
		...
		if (ModuleName) queryParams.append('ModuleName', ModuleName)
		if (ProcessOwnerName)
			queryParams.append('ProcessOwnerName', ProcessOwnerName)
	...
	const fetchProcess = async () => {
		...
		try {
			const response = await axios.get(
				`${config.API_URL_APPLICATION}/ProcessMaster/GetProcess`,
				{
					params: {
		...
		<>
			<div className="d-flex bg-white p-2 my-2 justify-content-between align-items-center fs-20">
				<span>
					...
					<span className="fw-bold test-nowrap">Process List</span>
				</span>
				<div className="d-flex">
					...
					{role === 'Admin' && (
						<Link to="/pages/ProcessMasterinsert">
							<Button variant="primary">Add Process</Button>
						</Link>
					)}
				</div>
			</div>
			<div className="bg-white p-2 pb-1">
				<Form
					...
					<Row>
						...
						<Col lg={6}>
							<Form.Group controlId="ModuleOwnerName">
								...
								<Select
									...
									onChange={(selectedOption) =>
										setProcessName(
											selectedOption ? selectedOption.processName : ''
										)
									} // null check
									options={processList}
						...
						<Col lg={6} className="mt-2">
							<Form.Group controlId="ProcessOwnerName">
								...
								<Select
									...
									value={
										employeeList.find(
											(item) => item.processOwnerName === ProcessOwnerName
										) || null
									} // handle null
									onChange={(selectedOption) =>
										setProcessOwnerName(
											selectedOption ? selectedOption.processOwnerName : ''
										)
									} // null check
						...
						<Col
							lg={4}
							className="align-items-end d-flex justify-content-end mt-3">
												...
												<Droppable droppableId="columns" direction="horizontal">
													{(provided) => (
														<tr
															...
															{role === 'Admin' && (
																<th>
																	<i className="ri-tools-line"></i> Action
																</th>
															)}
														</tr>
													)}
													...
													processes.slice(0, 10).map((item, index) => (
														<tr key={item.id}>
																			...
																			<td>
																				...
																				{item.userUpdatedMobileNumber ? (
																					<p className=" fw-normal m-0">
																						<a
																							href={`tel:${item.userUpdatedMobileNumber}`}>
																							{' '}
															...
															<td className="text-center">
																<Button
																	variant="primary"
																	className=" text-white icon-padding"
																	title="You can Add and Remove Porject from this Process."
																	onClick={() => handleEdit(item.id)}>
																	{' '}
																	<i className="fs-18 ri-folder-add-line "></i>{' '}
																</Button>
															</td>
															<td className="text-center">
																<Button
																	variant="primary"
																	className=" text-white icon-padding"
																	title="You can View the running tasks."
																	onClick={() => handleViewEdit(item.id)}>
																	{' '}
																	<i className=" fs-18 ri-eye-line "></i>
																</Button>
															</td>
															{role === 'Admin' && (
																<td className="text-center">
																	<Link
																		to={`/pages/ProcessMasterinsert/${item.id}`}>
																		<Button
																			variant="primary"
																			className=" text-white icon-padding"
																			title="You can Edit the Porcess.">
																...
																<Row className="justify-content-center">
																	<Col xs={12} md={8} lg={6}>
																		<Alert
																			...
																			<h4>No Task Found</h4>
																			<p>
																				You currently don't have Completed tasks
																			</p>
																		</Alert>
																	</Col>
																</Row>
