...
const HrInputMasterinsert = () => {
    ...
    const fetchModuleById = async (id: string) => {
        try {
            ...
            if (response.data.isSuccess) {
                const fetchedModule = response.data.hRInputs[0];
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            if (editMode) {
                await axios.post(`${config.API_URL_APPLICATION}/HRInputMaster/InsertorUpdateHRInput`, payload);
                navigate('/pages/HrInputMaster', { state: { 
                    successMessage:"HrInput Updated successfully!",
                   } });
            } else {
                await axios.post(`${config.API_URL_APPLICATION}/HRInputMaster/InsertorUpdateHRInput`, payload);
                navigate('/pages/HrInputMaster', { state: { 
                    successMessage:"HrInput Added successfully!",
                   } });
            }
        ...
        <div>
            <div className="container">
                <div className="d-flex bg-white p-2 my-2 justify-content-between align-items-center fs-20 rounded-3 border">
                    <span><i className="ri-file-list-line me-2"></i><span className='fw-bold'>{editMode ? 'Edit Hr Input' : 'Add Hr Input'}</span></span>
                </div>
                            ...
                            <Col lg={2} className='align-items-end d-flex justify-content-end mb-3'>
                                <ButtonGroup aria-label="Basic example" className='w-100'>
                                    ...
                                    <Button variant="primary" type="submit">
                                        {editMode ? 'Update Hr Input' : 'Add Hr Input'}
                                    </Button>
                                </ButtonGroup>
                            </Col>
