...
const HrTaskMaster = () => {
    ...
    const [downloadCsv, setDownloadCsv] = useState<HrTask[]>([]);
    ...
    //             setHrTasks(response.data.moduleMasterListResponses)
    //         })
    //         .catch((error) => {
    ...
    const fetchModules = async () => {
        ...
        try {
            ...
            if (response.data.isSuccess) {
                setHrTasks(response.data.hRTasks);
    ...
    const fetchModulesCsv = async () => {
        try {
            ...
            if (response.data.isSuccess) {
                setDownloadCsv(response.data.hRInputs);
            } else {
                console.error(response.data.message);
            }
        } catch (error) {
    ...
    const convertToCSV = (data: HrTask[]) => {
        const csvRows = [
    ...
    return (
        <>
            <div className="d-flex bg-white p-2 my-2 justify-content-between align-items-center fs-20">
                <span><i className="ri-file-list-line me-2"></i><span className='fw-bold test-nowrap'>Hr Task List</span></span>
                <div className="d-flex">
                    <Link to='/pages/HrTaskMasterinsert'>
                                                            ...
                                                            {columns.filter(col => col.visible).map((col) => (
                                                                ...
                                                                >
                                                                    <div>
                                                                        ...
                                                                        <td>{item[col.id as keyof HrTask]}</td>
                                                                    </div>
                                                                </td>
                                                            ))}
                                                            <td><Link to={`/pages/HrTaskMasterinsert/${item.id}`}>
