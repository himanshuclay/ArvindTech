...
const ProjectMasterPage: React.FC = () => {
    const [projectMaster, setProjectMaster] = useState<ProjectMaster>({
        id: 0,
        projectName: '',
        projectID: '',
        stateId: 0,
    ...
    const [projectMasterList, setProjectMasterList] = useState<ProjectMaster[]>([]);
    ...
    useEffect(() => {
        fetchProjectMaster(currentPage, rowsPerPage);
    }, [currentPage, rowsPerPage]);

    const fetchProjectMaster = async (page: number, rows: number) => {
        setLoading(true);
        try {
            const params = new URLSearchParams({ id: '1', PageIndex: page.toString() });
            ...
            if (response && response.status === 200) {
                setProjectMasterList(response.data.projectMasterList);
                setTotalCount(response.data.totalCount);
            } else {
                console.error('Failed to fetch project master list: Invalid response status');
            }
        } catch (error) {
            console.error('An error occurred while fetching the project master list:', error);
            if (error.response) {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        const payload = { ...projectMaster, updatedBy: projectMaster.createdBy };

        try {
            let response;
            if (editingIndex !== null) {
                // Update existing project master
                response = await axios.post('https://localhost:44306/api/ProjectMaster/UpdateProject', payload, {
                    headers: {
                        'accept': '*/*',
                ...
                response = await axios.post('https://localhost:44306/api/ProjectMaster/InsertProject', payload, {
                    headers: {
                        'accept': '*/*',
                        'Content-Type': 'application/json'
                    }
            ...
            if (response.status === 200 || response.status === 201) {
                const newProjectMaster = response.data;

                if (editingIndex !== null) {
                    const updatedProjectMasterList = [...projectMasterList];
                    updatedProjectMasterList[editingIndex] = newProjectMaster;
                    setProjectMasterList(updatedProjectMasterList);
                } else {
                    setProjectMasterList([...projectMasterList, { ...newProjectMaster, id: projectMasterList.length + 1 }]);
                }
                handleClose();
            } else {
                console.error('Failed to submit project master');
            }
        } catch (error) {
            if (axios.isAxiosError(error) && error.response) {
                console.error('An error occurred while submitting the project master:', error.response.data);
            } else {
                console.error('An error occurred while submitting the project master:', error);
            }
        }
    };
    ...
    const handleEdit = (index: number) => {
        ...
        setProjectMaster(projectMasterList[index]);
        handleShow();
    };
    ...
    const convertToCSV = (data: ProjectMaster[]) => {
        const csvRows = [
            ['Project Name', 'Project ID', 'State ID', 'Project Type', 'Management Contract', 'Project Incharge', 'Project Coordinator', 'Completion Status', 'Name of Work', 'Created By'],
            ...data.map(project => [
                project.projectName,
                project.projectID,
                project.stateId.toString(),
                project.projectType.toString(),
                project.managementContract.toString(),
                project.projectIncharge.toString(),
                project.projectCoordinator.toString(),
                project.completionStatus.toString(),
                project.nameOfWork,
                project.createdBy
            ])
        ];
    ...
    const downloadCSV = () => {
        const csvData = convertToCSV(projectMasterList);
        const blob = new Blob([csvData], { type: 'text/csv' });
        const url = URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url;
        a.download = 'project_masters.csv';
    ...
    return (
        <div className="container">
            <div className="d-flex bg-white p-2 my-2 justify-content-between align-items-center">
                <span><i className="ri-projector-line me-2"></i><span className='fw-bold'>Project Master List</span></span>
                <div className="d-flex">
                    ...
                    <Button variant="primary" onClick={handleShow}>
                        <i className="ri-add-line align-bottom me-1"></i> Add Project Master
                    </Button>
            ...
            {loading ? (
                ...
                <Table striped bordered hover responsive>
                    ...
                    <tbody>
                        {projectMasterList.map((project, index) => (
                            <tr key={index}>
                                <td>{project.projectName}</td>
                                <td>{project.projectID}</td>
                                <td>{project.stateId}</td>
                                <td>{project.projectType}</td>
                                <td>{project.managementContract}</td>
                                <td>{project.projectIncharge}</td>
            ...
            <Offcanvas show={show} onHide={handleClose} placement="end">
                <Offcanvas.Header closeButton>
                    <Offcanvas.Title>{editingIndex !== null ? 'Edit Project Master' : 'Add Project Master'}</Offcanvas.Title>
                </Offcanvas.Header>
                <Offcanvas.Body>
                    <Form onSubmit={handleSubmit}>
                        ...
                        <Form.Group className="mb-3">
                            ...
                            <Form.Control
                                ...
                                name="projectType"
                                value={projectMaster.projectType}
                                onChange={handleChange}
                                required
                            />
                        </Form.Group>
                        <Form.Group className="mb-3">
                            ...
                            <Form.Control
                                type="number"
                                placeholder="Enter Management Contract"
                                name="managementContract"
                                value={projectMaster.managementContract}
                                onChange={handleChange}
                                required
                            />
                        </Form.Group>
                        <Form.Group className="mb-3">
                            ...
                            <Form.Control
                                ...
                                value={projectMaster.projectIncharge}
                                onChange={handleChange}
                                required
                            />
                        </Form.Group>
                        <Form.Group className="mb-3">
                            ...
                            <Form.Control
                                ...
                                value={projectMaster.projectCoordinator}
                                onChange={handleChange}
                                required
                            />
                        </Form.Group>
                        <Form.Group className="mb-3">
                            ...
                            <Form.Control
                                ...
                                value={projectMaster.completionStatus}
                                onChange={handleChange}
                                required
                            />
                        </Form.Group>
                        <Form.Group className="mb-3">
                            ...
                            <Form.Control
                                ...
                                name="nameOfWork"
                                value={projectMaster.nameOfWork}
                                onChange={handleChange}
                                required
                            />
                        </Form.Group>
                        <Form.Group className="mb-3">
                            ...
                            <Form.Control
                                ...
                                name="createdBy"
                                value={projectMaster.createdBy}
                                onChange={handleChange}
                                required
                            />
                        </Form.Group>
                        <Button variant="primary" type="submit" className="me-2">
                            {editingIndex !== null ? 'Update Project Master' : 'Add Project Master'}
                        </Button>
